...
| Concern | File |
| --- | --- |
| Prisma model (API schema) | `apps/api/prisma/schema.prisma` — `BloomeLead` |
| Table migration | `apps/api/prisma/migrations/20260611000000_add_bloome_leads` |
| Data backfill (ACT:Live snapshot, idempotent) | `apps/api/prisma/migrations/20260611000001_backfill_bloome_act_live` |
| Source CSV snapshot | `apps/api/prisma/data/bloome-act-live-2026-06-10.csv` |
| API endpoints — `GET /leads/bloome`, `GET /leads/bloome/summary` | `apps/api/src/leads/bloome.controller.ts`, `bloome.service.ts` |
| Live sheet sync (poller + `POST /leads/bloome/sync` + `GET /leads/bloome/sync/status`) | `apps/api/src/leads/bloome-sync.service.ts` |
| Google-side bridge (Apps Script web app + setup steps) | `apps/api/scripts/bloome-sheets-webapp.gs` |
| Sync env config | `apps/api/.env.example` — `BLOOME_SYNC_*` |
| Client tab (KPIs, region sub-tabs, search, filters, pagination, Sync now, 60 s auto-refresh) | `apps/web/src/components/leads/bloome-leads-tab.tsx` |

Rows are imported from the Bloome **"ASTRA - MASTER BLASTER"** Google Sheet,
one `BloomeLead` per dialled lead, deduped on `(sourceTab, rowNum)` so
re-importing a newer snapshot only adds new rows. These records are
deliberately separate from the managed `Lead` pipeline (they are unowned,
free-form setter rows); future regions (e.g. `TAS:Live`) import into the same
table with a different `region`/`sourceTab` and appear automatically as a new
region sub-tab.
